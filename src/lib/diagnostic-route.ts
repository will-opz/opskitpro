...
export type DiagnosticExecutionOptions = {
  rateLimitRoute?: string;
  rateLimit?: number;
};

export async function executeDiagnosticRequest(
  ...
  const rateLimitRoute = options.rateLimitRoute || "/api/diagnostic";
  const rateLimit = checkRateLimit({
    ip,
    route: rateLimitRoute,
    costClass: "HIGH",
    limit: options.rateLimit ?? 3,
  });

  if (!rateLimit.success) {
    // Log abuse patterns (IP hash, route, status) specifically for denied requests
    console.warn(`[RATE_LIMIT] denied ${rateLimit.ipHash} on ${rateLimitRoute} (HIGH). Retry in ${rateLimit.retryAfterSeconds}s.`);
...
export async function GET(request: NextRequest | Request) {
  return executeDiagnosticRequest(request);
}
