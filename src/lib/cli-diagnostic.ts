...
export async function handleCliDiagnostic(request: NextRequest, domain?: string | null) {
  ...
  try {
    const diagnosticRequest = new NextRequest(
      `${request.nextUrl.origin}/api/diagnostic?domain=${encodeURIComponent(target)}`,
      { headers: request.headers },
    );
    const { GET: runDiagnostic } = await import("@/app/api/diagnostic/route");
    const response = await runDiagnostic(diagnosticRequest);
