...
type BreachCheckOptions = {
  cryptoSource?: Pick<Crypto, "subtle">;
...
export async function checkPwnedPassword(
  ...
  const fetchSource = options.fetchSource ?? globalThis.fetch;
  ...
  if (body.length > 2_000_000) throw new Error("Breach response is too large");

  for (const line of body.split(/\r?\n/)) {
    const match = /^([A-F0-9]{35}):(\d+)$/.exec(line.trim());
