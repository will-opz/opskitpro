...
const labels = {
  ...
  en: {
    ...
    fallbackHint:
      "Fallback for local development or when Zero Trust is unavailable.",
    divider: "or",
    login: "Sign in",
    cancel: "Cancel",
...
export function AdminSessionProvider({
  ...
  const [provider, setProvider] = useState<
    "cloudflare_access" | "password" | null
  >(null);
  ...
  const loadSession = async () => {
    ...
    return fetch("/api/admin/session")
      ...
      .then((data) => {
        ...
        setProvider(
          data.provider === "cloudflare_access" || data.provider === "password"
            ? data.provider
            : null,
        );
...
function AdminLoginDialog({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center px-4 py-6">
      ...
      <form
        onSubmit={submit}
        className="relative z-10 w-full max-w-md rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-elevated)] p-5 shadow-2xl"
      >
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-[var(--text-muted)]">
              {configured ? t.help : t.notConfigured}
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          {passwordConfigured && (
            <button
              type="submit"
              disabled={submitting || !configured || !email.trim() || !password}
