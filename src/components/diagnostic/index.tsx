...
export function MetaRow({ label, value, mono = false }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="flex items-center justify-between py-2 border-b border-[var(--border-subtle)] last:border-0">
      ...
      <span className={`text-xs font-semibold text-[var(--text-primary)] ${mono ? 'font-mono' : ''} max-w-[60%] text-right truncate`}>
        {value || '—'}
      </span>
    </div>
  )
...
export function StatusBadge({ status }: { status: 'ok' | 'slow' | 'failed' | 'info' }) {
  const map = {
    ok: 'bg-emerald-500',
    slow: 'bg-amber-400',
    failed: 'bg-red-400',
