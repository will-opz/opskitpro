...
const text = {
  ...
  en: {
    ...
    extensionItems: ['Live status source', 'Recent visits', 'Obsidian quick note'],
    scenesTitle: 'Scenes',
    scenes: {
      ops: 'Ops',
      ai: 'AI Hub',
  ...
  ja: {
    ...
    extensionItems: ['ライブ状態ソース', '最近のアクセス', 'Obsidian クイックメモ'],
    scenesTitle: '用途',
    scenes: {
      ops: '運用 Ops',
      ai: 'AI ハブ',
...
export default function ServicesClient({ dict, lang }: { dict: any, lang: Lang }) {
  ...
  const categorizedServices: ToolCategory[] = useMemo(() => [
    ...
    {
      category: isJapanese ? 'コミュニケーション基盤' : (lang === 'zh' ? '通信与协作协议' : lang === 'tw' ? '通訊與協作協定' : 'Communication Protocols'),
      scene: 'knowledge',
      tools: [
        { name: dict.tools.matrix_title, desc: dict.tools.matrix_desc, icon: MessageSquare, status: 'operational', url: 'https://matrix.org', scene: 'knowledge', tags: ['chat', 'secure', 'federation'], favorite: true, external: true, accent: 'emerald' },
      ],
    },
    {
  ...
  }, [activeScene, categorizedServices, copy.scenes, primaryTools, searchTerm, showPrimaryTools])
    ...
    <main className="w-full flex-grow text-[var(--text-primary)]">
      <section className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 pb-20 pt-8 sm:px-6 md:pt-10">
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_300px]">
          <div className="ui-surface-elevated rounded-[2rem] p-4 sm:p-5">
            <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
              <div className="max-w-3xl">
                <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-emerald-500/20 bg-emerald-500/8 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.24em] text-[var(--accent-color)]">
              ...
              <div className="grid grid-cols-3 gap-2 xl:w-[330px]">
                {[
                  ...
                  { label: copy.available, value: allTools.length, icon: CheckCircle2 },
                ].map((item) => {
                  const Icon = item.icon
                  return (
                    <div key={item.label} className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-secondary)] p-2.5">
                      <Icon className="mb-2 h-4 w-4 text-emerald-600" />
                      <div className="text-xl font-black text-[var(--text-primary)]">{item.value}</div>
                      <div className="mt-1 truncate text-[10px] font-semibold uppercase tracking-[0.18em] text-[var(--text-faint)]">{item.label}</div>
                    </div>
                  )
                })}
              </div>
            ...
            <div className="mt-5 grid gap-3 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-center">
              <label htmlFor="dashboard-search" className="relative block">
                ...
                />
                <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-tertiary)] px-2 py-1 text-[10px] font-semibold text-[var(--text-faint)]">
                  ⌘K
                </span>
              </label>
                    ...
                    <button
                      ...
                      className={`shrink-0 rounded-xl border px-3 py-1.5 text-xs font-semibold transition ${
                        isActive
                          ...
                          : 'border-[var(--border-subtle)] bg-[var(--surface-secondary)] text-[var(--text-secondary)] hover:border-emerald-500/30 hover:bg-[var(--accent-soft)] hover:text-[var(--accent-color)]'
                      }`}
                    >
                      {label}
                    </button>
          ...
          {showPrimaryTools && <section aria-labelledby="quick-launch" className="ui-surface order-2 rounded-[1.5rem] p-4 sm:p-5 lg:order-3 lg:col-span-2">
            <div className="mb-4 flex items-center justify-between gap-3">
              <h2 id="quick-launch" className="text-sm font-bold uppercase tracking-[0.2em] text-[var(--text-faint)]">
                {copy.quick}
              </h2>
            </div>
            <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
              {primaryTools.map((tool) => (
                <ToolLink key={tool.name} tool={tool} openLabel={copy.open} builtInLabel={copy.builtIn} featured />
              ))}
            </div>
          </section>}
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
          <div className="space-y-8">
            {filteredServices.length === 0 ? (
              <div className="flex min-h-[320px] flex-col items-center justify-center rounded-[1.5rem] border border-dashed border-[var(--border-subtle)] bg-[var(--surface-primary)] text-center">
                ...
                <p className="text-sm text-[var(--text-muted)]">{copy.noResult}</p>
              </div>
            ) : (
                ...
                <section key={cat.category} className="scroll-mt-24">
                  <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
                    <div>
                      ...
                      <h2 className="mt-1 text-xl font-black tracking-tight text-[var(--text-primary)]">{cat.category}</h2>
                    </div>
                  ...
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
                    {cat.tools.map((tool) => (
                      <ToolLink key={`${cat.category}-${tool.name}`} tool={tool} openLabel={copy.open} builtInLabel={copy.builtIn} />
                    ))}
                  </div>
                </section>
              ))
          ...
          <aside className="space-y-4 lg:sticky lg:top-8 lg:self-start">
            <Panel title={copy.extension} icon={Layers}>
              <p className="text-sm leading-6 text-[var(--text-muted)]">{copy.extensionCopy}</p>
              <div className="mt-4 space-y-2">
                {copy.extensionItems.map((item) => (
                  <div key={item} className="rounded-xl border border-dashed border-[var(--border-subtle)] bg-[var(--surface-secondary)] px-3 py-2 text-xs font-semibold text-[var(--text-muted)]">
                    {item}
                  </div>
                ))}
              </div>
  ...
  return (
    <section className="ui-surface rounded-[1.5rem] p-5">
      <div className="mb-4 flex items-center gap-3">
        ...
        <h2 className="text-sm font-bold uppercase tracking-[0.18em] text-[var(--text-secondary)]">{title}</h2>
      </div>
      {children}
    </section>
  )
...
function ToolLink({ tool, compact = false, featured = false, openLabel = 'Open', builtInLabel = 'Built in' }: { tool: Tool; compact?: boolean; featured?: boolean; openLabel?: string; builtInLabel?: string }) {
    ...
    return (
      ...
      >
        <div className="flex items-start justify-between gap-2">
          ...
          {external && <ExternalLink className="h-3.5 w-3.5 text-[var(--text-faint)] transition group-hover:text-[var(--accent-color)]" />}
        </div>
        <div>
          <h3 className="truncate text-sm font-bold text-[var(--text-primary)]">{tool.name}</h3>
          <p className="mt-1 truncate text-xs text-[var(--text-muted)]">{tool.tags.slice(0, 2).join(' / ')}</p>
        </div>
      </Link>
    )
  }
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex min-w-0 items-start gap-3">
          <div className={`flex shrink-0 items-center justify-center rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-tertiary)] text-[var(--text-secondary)] transition group-hover:border-emerald-500/20 group-hover:bg-emerald-500/8 group-hover:text-[var(--accent-color)] ${featured ? 'h-12 w-12' : 'h-11 w-11'}`}>
            <Icon className={featured ? 'h-6 w-6' : 'h-5 w-5'} />
          </div>
          <div className="min-w-0">
            <h3 className="truncate text-base font-bold text-[var(--text-primary)]">{tool.name}</h3>
            <p className="mt-2 text-sm leading-6 text-[var(--text-muted)]">{tool.desc}</p>
          </div>
        </div>
        {external && <ExternalLink className="h-4 w-4 shrink-0 text-[var(--text-faint)] transition group-hover:text-[var(--accent-color)]" />}
      </div>

      <div className="mt-5 flex items-center justify-between gap-3">
        <div className="flex flex-wrap gap-1.5">
          {tool.tags.slice(0, 3).map((tag) => (
            <span key={tag} className="rounded-full border border-[var(--border-subtle)] bg-[var(--bg-tertiary)] px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--text-muted)]">
              {tag}
            </span>
          ))}
        </div>
        <span className={`inline-flex shrink-0 items-center rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.14em] ${
          builtIn
            ? 'border-emerald-500/20 bg-emerald-500/8 text-[var(--accent-color)]'
  ...
  return (
    ...
    >
      <span className="flex min-w-0 items-center gap-3">
        <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-secondary)] text-[var(--text-secondary)] transition group-hover:border-emerald-500/20 group-hover:text-[var(--accent-color)]">
          <Icon className="h-4 w-4" />
        </span>
        <span className="min-w-0">
          <span className="block truncate font-semibold text-[var(--text-primary)]">{tool.name}</span>
          <span className="block truncate text-xs text-[var(--text-muted)]">{tool.tags.slice(0, 2).join(' / ')}</span>
        </span>
      </span>
      {external && <ExternalLink className="h-3.5 w-3.5 shrink-0 text-[var(--text-faint)] transition group-hover:text-[var(--accent-color)]" />}
    </Link>
  )
}
