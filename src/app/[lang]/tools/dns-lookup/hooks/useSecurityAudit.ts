  ...
  const runAudit = async (domain: string) => {
    ...
    try {
      ...
      const [spfRes, dmarcRes, caaRes] = await Promise.all([
        ...
        fetchApi(
          `/api/dns?domain=_dmarc.${encodeURIComponent(cleanDomain)}&type=TXT`,
        ),
      ...
      const spfRecords = txtAnswers
        .map((a: any) => a.data)
        .filter((txt: string) => txt.includes("v=spf1"));
      ...
      if (spfRecords.length === 0) {
        ...
        spfStatus = "fail";
        spfMessage =
          "Multiple SPF records found. This will cause authentication to fail.";
      } else {
        ...
        if (spf.includes("-all")) {
          ...
          spfStatus = "warning";
          spfMessage =
            "Soft fail (~all) is configured. This is common but less strict than -all.";
        } else if (spf.includes("?all") || spf.includes("+all")) {
          spfStatus = "fail";
          spfMessage =
            "Weak SPF policy (?all or +all). This provides little to no protection against spoofing.";
        } else {
          spfStatus = "warning";
          spfMessage =
            "SPF record exists, but default policy (all) is unclear or missing.";
        }
      }

      // 2. Parse DMARC
      const dmarcAnswers = dmarcRes.answers || [];
      const dmarcRecords = dmarcAnswers
        .map((a: any) => a.data)
        .filter((txt: string) => txt.includes("v=DMARC1"));
      ...
      if (dmarcRecords.length === 0) {
        ...
        if (dmarcLower.includes("p=reject")) {
          ...
          dmarcMessage =
            "DMARC is set to quarantine. Suspicious emails may go to spam.";
        } else if (dmarcLower.includes("p=none")) {
          dmarcStatus = "info";
          dmarcMessage =
            "Monitoring mode (p=none) is active. Recommend reviewing RUA reports and upgrading to quarantine/reject.";
        } else {
          dmarcStatus = "warning";
      ...
      const passes = [spfStatus, dmarcStatus].filter(
        (s) => s === "pass",
      ).length;
