...
export const createSafeDiagnosticResult = (
  ...
  return {
    ...
    ssl: {
      ...
      date_valid: data?.ssl?.date_valid,
      hostname_valid: data?.ssl?.hostname_valid,
      chain_authorized: data?.ssl?.chain_authorized,
      ocsp_stapled: data?.ssl?.ocsp_stapled,
      issuer: data?.ssl?.issuer || "Unknown",
      expiry: data?.ssl?.expiry || "Unknown",
      subject_alt_name: data?.ssl?.subject_alt_name,
      grade: data?.ssl?.grade || "—",
      factors: data?.ssl?.factors || [],
      tls_version: data?.ssl?.tls_version,
      protocol: data?.ssl?.protocol,
      cipher: data?.ssl?.cipher,
      alpn: data?.ssl?.alpn,
      legacy_tls_accepted: data?.ssl?.legacy_tls_accepted,
      error_reason: data?.ssl?.error_reason,
