...
export function repairJson(input: string): RepairResult {
  ...
  if (missingCommasObj && missingCommasObj.length > 0) {
    ...
    fixes.push(
      `Added ${missingCommasObj.length} missing comma(s) between structures`,
    );
  }
  ...
  if (missingCommasVal && missingCommasVal.length > 0) {
    ...
    fixes.push(
      `Added ${missingCommasVal.length} missing comma(s) before keys/values`,
    );
  }
  ...
  const unquotedValPattern =
    /(?<=:\s*)(?!(?:true|false|null|undefined|NaN))([a-zA-Z_$][a-zA-Z0-9_$]*)(?=\s*[,}\]])/g;
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    if (ch === "{" || ch === "[") {
      ...
      if (
        ...
        openerStack[openerStack.length - 1].ch === "{"
      ) {
        openerStack.pop();
      } else {
  ...
  if (unmatchedClosers.length > 0) {
    ...
    fixes.push(
      `Removed ${unmatchedClosers.length} extra bracket(s) "${removed}"`,
    );
  }

  if (openerStack.length > 0) {
    const closers = openerStack
      .map((o) => (o.ch === "{" ? "}" : "]"))
      .reverse()
      .join("");
    s += closers;
