...
export function useJqQuery() {
  ...
  const runQuery = useCallback(
    async (json: string, filter: string): Promise<JqResult> => {
      if (!isReady || !jqRef.current) {
        return {
          output: "",
          error: "JQ engine not ready",
          duration: 0,
          matchCount: 0,
        };
      }
...
export const JQ_SNIPPETS = [
  ...
  { label: "Map names", filter: ".[].name", desc: "Extract name field" },
  {
    ...
    desc: "Filter by condition",
  },
  {
    label: "Group by",
    filter: "group_by(.type)",
    desc: "Group array elements",
  },
  {
    label: "Count",
    filter: "[.[] | .type] | group_by(.) | map({key: .[0], count: length})",
    desc: "Count by field",
  },
];
