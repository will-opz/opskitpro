        ...
        <div className="px-6 py-3 bg-amber-50/80 border-b border-amber-100 flex flex-col sm:flex-row sm:items-center gap-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] font-mono font-bold text-amber-700 uppercase tracking-wider">
              {dict.tools.json.repair_summary ||
                `${repairFixes.length} fix(es) applied`}
            </span>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {repairFixes.map((fix, i) => (
              <span
                key={i}
                className="text-[10px] font-mono bg-amber-100 text-amber-800 px-2 py-0.5 rounded-full border border-amber-200"
              >
                {fix}
              </span>
            ))}
        ...
        {diffData.map((line, idx) => {
          if (line.type === "same") {
            return (
              <div key={idx} className="flex">
                <span className="w-10 text-right pr-3 text-zinc-300 select-none shrink-0">
                  {idx + 1}
                </span>
                <span className="text-zinc-600 whitespace-pre">
                  {line.text}
                </span>
              </div>
            );
          ...
          if (line.type === "removed") {
            return (
              <div
                key={idx}
                className="flex bg-red-50 border-l-2 border-red-400 rounded-r"
              >
                <span className="w-10 text-right pr-3 text-red-300 select-none shrink-0">
                  −
                </span>
                <span className="text-red-700 whitespace-pre line-through opacity-70">
                  {line.text}
                </span>
              </div>
            );
          ...
          return (
            <div
              key={idx}
              className="flex bg-emerald-50 border-l-2 border-emerald-400 rounded-r"
            >
              <span className="w-10 text-right pr-3 text-emerald-400 select-none shrink-0">
                +
              </span>
              <span className="text-emerald-800 whitespace-pre font-semibold">
                {line.text}
              </span>
            </div>
          );
