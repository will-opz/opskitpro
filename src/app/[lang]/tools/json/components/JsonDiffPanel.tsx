...
export function JsonDiffPanel({ leftJson }: JsonDiffPanelProps) {
  ...
  const filteredResults = useMemo(() => {
    ...
    return showSame
      ? diffResults
      : diffResults.filter((d) => d.type !== "same");
  ...
  const getTypeIcon = (type: DiffResult["type"]) => {
    switch (type) {
      ...
      case "same":
        return <Equal className="w-3.5 h-3.5 text-zinc-400" />;
    }
  };
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-zinc-100 overflow-hidden">
        ...
        <div className="flex flex-col overflow-hidden">
          ...
          <pre className="flex-1 p-4 overflow-auto font-mono text-[11px] leading-relaxed text-zinc-600 bg-white">
            {leftJson || (
              <span className="text-zinc-300 italic">Input JSON in editor</span>
            )}
          </pre>
        </div>
        ...
        <div className="border-t border-zinc-200 max-h-[300px] overflow-auto">
          <div className="px-4 py-2 bg-zinc-50 border-b border-zinc-100 sticky top-0">
            <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-zinc-500">
              {filteredResults.length} Difference
          ...
          <div className="divide-y divide-zinc-50">
            {filteredResults.map((diff, idx) => (
              <div
                key={idx}
                className={`px-4 py-2 ${getTypeBg(diff.type)} border-l-2`}
              >
                <div className="flex items-start gap-2">
                  {getTypeIcon(diff.type)}
                  <div className="flex-1 min-w-0">
                    <code className="text-[11px] font-mono font-semibold text-zinc-700">
                      {diff.path}
                    </code>
                    {diff.type === "changed" && (
