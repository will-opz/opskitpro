...
export function useMultiConnection() {
  ...
  const updateTab = useCallback(
    (tabId: string, updates: Partial<ConnectionTab>) => {
      setTabs((prev) =>
  ...
  const addLog = useCallback(
    (
      ...
      type: LogEntry["type"],
      message: string,
      options?: { messageType?: LogEntry["messageType"] },
    ) => {
  ...
  const connect = useCallback(
    (tabId: string, url: string, config?: Partial<WebSocketConfig>) => {
          ...
          if (fullConfig.autoReconnect) {
            ...
            if (attempts < fullConfig.maxReconnectAttempts) {
              ...
              addLog(
                tabId,
                "info",
                `Reconnecting in ${fullConfig.reconnectInterval / 1000}s...`,
              );
              const timer = setTimeout(
                () => connect(tabId, url, fullConfig),
  ...
  const send = useCallback(
    (tabId: string, data: string | ArrayBuffer) => {
      const socket = socketsRef.current.get(tabId);
      ...
      try {
        ...
        addLog(
          ...
          {
            messageType:
              data instanceof ArrayBuffer ? "binary" : isJson ? "json" : "text",
          },
        );
        return true;
      } catch (e: any) {
