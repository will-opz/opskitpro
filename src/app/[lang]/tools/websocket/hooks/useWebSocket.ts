...
export function useWebSocket() {
  ...
  const connect = useCallback(
    (url: string, options?: Partial<WebSocketConfig>) => {
      ...
      try {
        ...
        ws.onclose = (event) => {
          ...
          if (
            ...
            reconnectAttemptRef.current < newConfig.maxReconnectAttempts
          ) {
            reconnectAttemptRef.current++;
            addLog(
              "info",
              `Reconnecting in ${newConfig.reconnectInterval / 1000}s (attempt ${reconnectAttemptRef.current}/${newConfig.maxReconnectAttempts})...`,
            );
