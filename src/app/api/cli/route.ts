...
`;
  ...
  try {
    ...
    const res = await fetch(
      `${origin}/api/diagnostic?domain=${encodeURIComponent(domain)}`,
      {
        // Pass a User-Agent to ensure the diagnostic API correctly processes the request
        headers: {
          "User-Agent":
            request.headers.get("User-Agent") || "OpsKitPro-CLI/1.0",
        },
      },
    );
    ...
    const currentHolds = data.whois?.status
      ? data.whois.status
          .split(",")
          .filter((s: string) => s.toLowerCase().includes("hold"))
      : [];
    const isCritical =
      !data.http.success || currentHolds.length > 0 || !data.ssl.valid;

    const stateStr = isCritical
      ? `${c.red}CRITICAL${c.reset}`
      : score >= 80
        ? `${c.green}NOMINAL${c.reset}`
        : `${c.yellow}DEGRADED${c.reset}`;
    const domainStatusStr =
      currentHolds.length > 0
        ? `${c.bgRed}${c.black} ${currentHolds.join(", ").toUpperCase()} ${c.reset}`
        : data.whois?.status
          ? data.whois.status.split(",")[0].toUpperCase()
          : "UNKNOWN";
    ...
    const ipStr =
      data.dns.all_ips && data.dns.all_ips.length > 0
    ...
    if (data.dns.resolvers) {
      const resolverSummary = data.dns.resolvers
        .map(
          (r: any) =>
    ...
    const gradeColor = data.ssl.grade?.startsWith("A")
      ? c.green
      : data.ssl.grade === "B"
        ? c.yellow
        : c.red;
    ...
    const hstsColor = data.ssl.factors?.includes("HSTS_ENABLED")
      ? c.green
      : c.yellow;
