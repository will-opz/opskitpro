  ...
  return (
    <main className="w-full flex-grow text-[var(--text-primary)]">
      <section className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 pb-20 pt-8 sm:px-6 md:pt-10">
        <div className="ui-surface-elevated rounded-[1.75rem] p-4 sm:p-6">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-emerald-500/20 bg-emerald-500/8 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.22em] text-[var(--accent-color)]">
                <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
                {t.badge}
              </div>
              <h1 className="text-3xl font-black tracking-tight sm:text-5xl">{t.title}</h1>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-[var(--text-muted)]">{t.subtitle}</p>
            </div>
          ...
          <div className="mt-6 grid gap-3 md:grid-cols-3">
            ...
            <StatCard label={t.customCount} value={customItems.length} icon={Edit3} />
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
          <label htmlFor="tool-search" className="relative block">
            ...
            <input
              ...
              className="h-12 w-full rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-primary)] pl-11 pr-4 text-sm text-[var(--text-primary)] outline-none placeholder:text-[var(--text-faint)] focus:border-emerald-500/60 focus:ring-4 focus:ring-emerald-500/10"
            />
          </label>
              ...
              <button
                ...
                className={`shrink-0 rounded-xl border px-3 py-2 text-xs font-semibold ${
                  activeCategory === category
                    ? 'border-emerald-500 bg-[var(--accent-color)] text-[var(--accent-contrast)]'
                    : 'border-[var(--border-subtle)] bg-[var(--surface-primary)] text-[var(--text-secondary)] hover:border-emerald-500/30 hover:bg-[var(--accent-soft)] hover:text-[var(--accent-color)]'
                }`}
              >
                {labels[category]}
              </button>
...
function StatCard({ label, value, icon: Icon }: { label: string; value: number; icon: typeof Activity }) {
  return (
    <div className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-secondary)] p-4">
      <Icon className="mb-3 h-4 w-4 text-emerald-500" />
      <div className="text-2xl font-black">{value}</div>
      <div className="mt-1 text-[10px] font-bold uppercase tracking-[0.18em] text-[var(--text-faint)]">{label}</div>
    </div>
  )
}
...
function NavCard({
  ...
  return (
    <article className="group flex min-h-[190px] flex-col justify-between rounded-[1.25rem] border border-[var(--border-subtle)] bg-[var(--surface-primary)] p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-emerald-500/30 hover:shadow-lg">
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 gap-3">
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-tertiary)] text-[var(--text-secondary)] group-hover:text-[var(--accent-color)]">
            <Icon className="h-5 w-5" />
          </div>
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <h2 className="truncate text-base font-bold">{item.title}</h2>
              {item.pinned && <Star className="h-3.5 w-3.5 shrink-0 fill-emerald-500 text-emerald-500" />}
            </div>
            <p className="mt-2 line-clamp-2 text-sm leading-6 text-[var(--text-muted)]">{item.description}</p>
          </div>
        </div>
        {external && <ExternalLink className="h-4 w-4 shrink-0 text-[var(--text-faint)]" />}
      </div>

      <div className="mt-5 space-y-4">
        <div className="flex flex-wrap gap-1.5">
          <span className="rounded-full border border-emerald-500/20 bg-emerald-500/8 px-2 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--accent-color)]">
        ...
        <div className="flex items-center justify-between gap-2">
          <Link
            ...
            className="inline-flex min-h-10 flex-1 items-center justify-center gap-2 rounded-xl bg-[var(--accent-color)] px-3 py-2 text-sm font-bold text-[var(--accent-contrast)] hover:bg-[var(--accent-hover)]"
          >
            {openLabel}
          ...
          {editable && (
            <div className="flex gap-1.5">
              <button type="button" onClick={() => onEdit(item)} className="flex h-10 w-10 items-center justify-center rounded-xl border border-[var(--border-subtle)] text-[var(--text-secondary)] hover:border-emerald-500/30 hover:text-[var(--accent-color)]" aria-label={editLabel}>
                <Edit3 className="h-4 w-4" />
              </button>
              <button type="button" onClick={() => onDelete(item)} className="flex h-10 w-10 items-center justify-center rounded-xl border border-[var(--border-subtle)] text-[var(--text-secondary)] hover:border-red-500/30 hover:text-red-500" aria-label={deleteLabel}>
