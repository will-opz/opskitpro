...
- **远程 MCP 网站检测** (`/mcp`)：让兼容的 AI 客户端调用有明确边界的
  Website Check，并获得结构化观察点、问题、证据、限制和下一步建议；接入说明
  位于 `/zh/mcp` 和 `/en/mcp`。
...
- 页面只保留当前会话最近 5 条生成记录，刷新或关闭页面后清除，不写入
  localStorage、IndexedDB 或服务端。
- 当前工具不提供密码管理、保险库、泄露检查、账号、找回或云同步。
...
- 生成的密码和强度分析不会发送到 OpsKitPro。
- 密码生成记录只存在于当前页面会话，不写入浏览器长期存储或服务端。
- 密码工具不会向第三方发送密码、哈希或泄露查询。
